...
anrr_weights = surveys %>% 
  ...
  mutate(
    ...
    site_category = fct_recode(site_category, "Encampment" = "Hotspot"),
    non_response = factor(
      non_response,
      levels = c(TRUE, FALSE),
...
anrr_folds = vfold_cv(training(anrr_split), v = 10, repeats = 5)
```
...
grid_results = models %>% 
  workflow_map(
    ...
    control = grid_ctrl,
    verbose = TRUE,
    metrics = metric_set(rmse, roc_auc, accuracy, mn_log_loss, pr_auc, average_precision, f_meas)
  )
write_rds(grid_results, "grid_results_with_weights.rds")
...
predictions = collect_predictions(grid_results)

predictions %>% 
  filter(wflow_id == "basic_logit_glm") %>% 
  ggplot(aes(x = non_response, y = `.pred_Non-Response`)) +
  geom_jitter()

best_predictions = collect_predictions(grid_results, select_best = TRUE, metric = "roc_auc")
```

```{r}
best_predictions %>% 
  ggplot(aes(x = `.pred_Non-Response`, y = wflow_id, fill = wflow_id)) +
  geom_density_ridges() +
  facet_wrap(vars(non_response), ncol = 1) +
  guides(fill = "none") +
  labs(x = "Predicted non-response probability", y = NULL)

best_predictions %>% 
  ggplot(aes(x = `.pred_Non-Response`, color = wflow_id)) +
  stat_ecdf()
...
best_predictions %>% 
  ...
  group_by(wflow_id, pr_bin) %>% 
  summarise(
    n = n(),
    avg_pred_prob = mean(`.pred_Non-Response`),
    pct_nonresponse = mean(non_response == "Non-Response")
  ) %>% 
  ggplot(aes(x = avg_pred_prob, y = pct_nonresponse, group = wflow_id, color = wflow_id)) +
  geom_line() +
  geom_point() +
